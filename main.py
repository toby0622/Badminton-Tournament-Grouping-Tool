...
from PySide6.QtWidgets import (
    ...
    QListWidget, QTextEdit, QMessageBox, QListWidgetItem
)
from PySide6.QtCore import Qt

# =============================================================================
# 核心賽程邏輯 (從原先的程式碼修改而來)
# 主要改動：generate...schedule 函式不再 print，而是 return 一個字串
# =============================================================================
...
class TournamentScheduler:
    ...
    def generate_knockout_schedule(self):
        ...
        if players_for_round1:
            ...
            for i in range(num_round1_matches):
                ...
                output.append(f"比賽 {i+1}: {self._format_participant(p1)} vs {self._format_participant(p2)}")
        else:
            output.append("\n所有參賽隊伍在第一輪輪空。")
        ...
        while len(round2_entrants) > 1:
            output.append(f"\n--- 第 {current_round} 輪 ---")
            if len(round2_entrants) == 4: output.append("(半決賽)")
            if len(round2_entrants) == 2: output.append("(決賽)")

            next_round_entrants = []
            num_matches_this_round = len(round2_entrants) // 2
            for i in range(num_matches_this_round):
                p1, p2 = round2_entrants[i], round2_entrants[len(round2_entrants) - 1 - i]
                output.append(f"比賽 {i+1}: {self._format_participant(p1)} vs {self._format_participant(p2)}")
    ...
    def generate_round_robin_schedule(self):
        ...
        for r in range(num_rounds):
            output.append(f"\n--- 第 {r+1} 輪 ---")
            p1_list, p2_list = local_participants[:half_size], local_participants[half_size:]
            p2_list.reverse()
...
class MainWindow(QMainWindow):
    ...
    def generate_schedule(self):
        try:
            # 1. 獲取所有選手
            all_players = [self.list_widget.item(i).text() for i in range(self.list_widget.count())]
            ...
            if self.radio_singles.isChecked():
                ...
                if self.radio_random_teams.isChecked():
                    ...
                    if left_out:
                        QMessageBox.information(self, "分隊提醒", f"選手 '{left_out}' 因人數為奇數，本次未被分到隊伍。")
                else: # 手動輸入隊伍
